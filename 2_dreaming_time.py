...
def extractVideo(inputdir, outputdir):
    cmd = ['ffmpeg -i ' + inputdir + ' -f image2 ' + outputdir + '/%08d.png']
...
def deepdream(net, base_img, image_type, iter_n=10, octave_n=4, octave_scale=1.4, end='inception_4c/output', verbose=1,
...
def deepdream_guided(net, base_img, image_type, iter_n=10, octave_n=4, octave_scale=1.4, end='inception_4c/output',
                     clip=True, verbose=1, objective_fn=objective_guide, **step_params):
    # if objective_fn is None:
    #    objective_fn = objective_L2
...
def main(input, output, image_type, gpu, model_path, model_name, preview, octaves, octave_scale, iterations, jitter,
         zoom, stepsize, blend, layers, guide_image, start_frame, end_frame, verbose):
    ...
    for i in range(frame_i, nrframes):
        ...
        if guide_image is None:
            frame = deepdream(net, frame, image_type=image_type, verbose=verbose, iter_n=iterations, step_size=stepsize,
                              octave_n=octaves, octave_scale=octave_scale, jitter=jitter, end=endparam)
        else:
            ...
            frame = deepdream_guided(net, frame, image_type=image_type, verbose=verbose, iter_n=iterations,
                                     ...
                                     end=endparam, objective_fn=objective_guide, guide_features=guide_features, )
...
if __name__ == "__main__":
    ...
    if args.extract is 1:
        ...
        main(args.input, args.output, args.image_type, args.gpu, args.model_path, args.model_name, args.preview,
             args.octaves, args.octavescale, args.iterations, args.jitter, args.zoom, args.stepsize, args.blend,
             args.layers, args.guide_image, args.start_frame, args.end_frame, args.verbose)
